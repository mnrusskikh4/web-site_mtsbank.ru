...
public class RampOfficesPage {
    ...
    private final String arrowDownDesktop = "svg[data-testid='icon_baseX24/ic-arrow-down']";

    // Локаторы для мобильной версии
    private final String listTabMobile = "//div[@data-testid='text' and @color='inherit' and contains(@class, 'Wrapper-sc-1vydk7-0') and contains(@class, 'TabText-sc-tyqhzb-3') and text()='Списком']";
    private final String arrowDownMobile = "//div[@data-testid='flexbox' and contains(@class, 'Inner-sc-1rfqasg-0') and contains(@class, 'jviKiO') and contains(@class, 'bqTsRY')]";
    private final String acceptButtonMobile = "//div[@data-testid='text' and text()='Применить']";


    // Общие локаторы
    private final String workingNowCheck = "//div[@data-testid='text' and text()='Работает сейчас']";
    private final String rampCheck = "//div[@data-testid='text' and text()='Есть пандус']";
    ...
    public RampOfficesPage clickCheckboxWorkingNow() {
        $x(workingNowCheck).shouldBe(Condition.visible, Condition.enabled).click();
        return this;
    }
    ...
    public RampOfficesPage clickCheckboxHasRamp() {
        $x(rampCheck).shouldBe(Condition.visible, Condition.enabled).click();
        return this;
    }
